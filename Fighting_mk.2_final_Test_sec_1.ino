...
//  motor2.setSpeed(0);
  ...
  else
  {
    ...
    else if(sensors[0].read()>sensors[1].read()+500 && sensors[1].read()<lim)
     {
    ...
    else if(sensors[0].read()<lim && sensors[1].read()>sensors[0].read()+500)
     {
    ...
    else if(sensors[0].read()<lim && sensors[1].read()<lim && diff <= 100)
    {
    
     
    // 일직선 상에서 인식 중 일정거리 이내 가까워 진 경우 최대 속도로 돌진 (중간에 물체가 사라진 경우 즉각적인 반응이 느려 무지성 돌진하다가 떨어지는 것을 방지하기 위함) 
    if(sensors[0].read()<lim2 || sensors[1].read()<lim2 && diff <= 100)
    {
